...
int main(int numparam, char* listaparam[]) {
	...
	scanf("%d", &tamanho);
	*/
	// Ao inves de ler por scanf o tamanho da memoria, este valor DEVE vir
	// pela linha de comando (chamada) do programa
	if (numparam < 2) {	// nao forneceu o tamanho?
		puts("NUMERO INVALIDO DE PARAMETROS");

		// A primeira string (o primeiro argumento) SEMPRE eh o nome do
		// programa como foi chamado da linha de comando
		printf("Use assim: %s <tamanho>.\n", listaparam[0]);
		return EXIT_FAILURE;
	}
	// Usa uma funcao da biblioteca-padrao para converter de string para
	// inteiro
	int tamanho = atoi(listaparam[1]);
